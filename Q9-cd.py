...
import starter.MAB_arms as ar
import starter.MAB_instance
import starter.MAB_policies
...
def draw_dist(arm, label=None, color=None):
    ...
    if color is not None:
        sns.displot(samples,label=label, color=color)
    else:
        sns.displot(samples,label=label)
    plt.xlabel('Reward')
...
#============
# Exercise 9.1 (d) Compare policies
arms = [weibull_arm, normal_arm]
policies = [starter.MAB_policies.EpsilonGreedy(0.1), starter.MAB_policies.UCB1()]
fig,ax = plt.subplots()
def test_policy(arms_, policy, n):
    trials, rewards = starter.MAB_instance.Instance(arms_, policy).sim_policy(n)
    sns.displot(rewards)
